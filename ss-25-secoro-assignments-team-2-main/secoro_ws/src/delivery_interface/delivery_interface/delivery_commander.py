...
ROOM_DOOR_LOCATIONS = {
    'room_251_door': {'x': -4.5, 'y': 0.5, 'yaw': 0.0},
    'room_252_door': {'x': 1.3, 'y': 0.5, 'yaw': 0.0},
    'room_253_door': {'x': 4.57, 'y': 0.5, 'yaw': 0.0},
    'room_254_door': {'x': 4.57, 'y': -0.5, 'yaw': 0.0},
    'room_255_door': {'x': 1.3, 'y': -0.5, 'yaw': 0.0},
    'room_256_door': {'x': -1.3, 'y': -0.5, 'yaw': 0.0},
    'room_257_door': {'x': -4.5, 'y': -0.5, 'yaw': 0.0},
    'room_test_door': {'x': -0.5, 'y': -0.5, 'yaw': 0.0},
}
...
class DeliveryCommander(Node):
    ...
    def continue_flow(self):
        if self.state == 'pickup':
            confirm = input("Sender: please confirm that the package has been loaded onto the turtlebot with [y]: ")
            if confirm.lower() == 'y':
            ...
            confirm = input("Receiver: Please confirm that package was delivered and picked up with [y]: ")
            if confirm.lower() == 'y':
                self.get_logger().info('returning to dockstation')
                self.target = self.load_room_waypoint('dock1')
                if self.target:
                    self.state = 'dock'
                    self.send_goal(self.target)
